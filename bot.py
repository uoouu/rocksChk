...
def chk_card(card):
    if len(card) > 20:
        ...
        if space in req:
            ...
            try:
                cc,mm,yy,cvv = card[0],card[1],card[2],card[3]
                if (len(cc) < 12 or len(cc) > 18) or (len(mm) > 2 or len(yy) < 1) or (len(yy) < 2 or len(yy) > 4 or len(yy)==3):
                    return(False)
                else:
                    return(True)
            except:
    ...
    if cr == None:
        ...
        if car != True:
            ...
            try:
                ...
                card = card.split(space)
                cc,mm,yy,cvv = card[0],card[1],card[2],card[3] 
                import braintree_auth3
                result = braintree_auth3.start(cc,mm,yy,cvv,use_proxy=False,proxy="none")
                user_states[id] = False
                if "card" in result:
...
*Bank*: {bank}
*Time*: {timee} seconds
*Remaining Credit ( {credit_remain} ) 💳*''',chat_id=id,message_id=start_msg.id,parse_mode="Markdown")
                    except:
                        bot.edit_message_text(text=f'''Card: {card} {em}
Bin: {card[0:6]}
Gateway: Braintree auth 3
...
Bank: {bank}
Time: {timee} seconds
Remaining Credit ( {credit_remain} ) 💳''',chat_id=message.chat.id,message_id=start_msg.id)
...
def send(message):
    if str(message.chat.id) == admin:
        ...
        try:
            ...
            pass
        try:
            file4 = open("braintree_auth3.py","br")
