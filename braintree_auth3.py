...
def log(req,user_agent,db):
    ...
    db.setex(key=email,value=True,ttl=40)
    try:
...
def delet(url_delet,req,user_agent):
...
def start(cc,mm,yy,cvv,use_proxy,proxy):
    try:
        ...
        if use_proxy == True:
            prox(proxy,req)
        db = Client("data.db")
        login = log(req,user_agent,db)
        db.close()
        if login == False:
            return{"error":f"error account."}
        ...
        if chk == "error_proxy":
            ...
            if url_delet != None:
                delet(url_delet,req,user_agent)
